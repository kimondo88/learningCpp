...
public:
    ...
    int CardsLeft(); // gets size of of cards left.
};
...
public:
    ...
    ~Game();
    void Play(); // plays the game of black jack.
private:
    Deck m_Deck;
    House m_House;
...
void Game::Play()
{
    ...
    for(pPlayer = m_Players.begin(); pPlayer != m_Players.end(); ++pPlayer)
    {
        if(m_Deck.CardsLeft() < 5)
        {
            //check if card deck are less than 5 card the repopulate deck.
            m_Deck.Populate();
        }
